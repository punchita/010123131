...
def make_Circle():
    for num in range(10):
        ...
        out = pygame.draw.circle(screen, color, (x,y), r ,0)
        namelist.append(out)
        createcircle.append((x,y,r))
        radius.append(r)
    return 0
...
def get_cir_rm():
    ...
    for x,y,r in cir_place :
        if r == max_r: 
            c = createcircle[createcircle.index((x,y,r))]   # Let c = circle with the most radius
            index = createcircle.index((x,y,r))          # find index of circle with the most radius
            circle = Namelist[index]                      # Let circle = val of  circle 
...
def  del_data():
    ...
    Namelist.remove(namelist[index])                  # Remove val from Name
    createcircle.remove(createcircle[index])        # Remove position from cir_place
